...
switch ($type) {
	case 'c':	// Category
		...
		if ($cat === null) {
			Minz_Error::error(FreshRSS_HttpResponseCode::HTTP_404_NOT_FOUND->value, "Category {$id} not found!");
			die();
		}
	...
	case 'f':	// Feed
		...
		if ($feed === null) {
			Minz_Error::error(FreshRSS_HttpResponseCode::HTTP_404_NOT_FOUND->value, "Feed {$id} not found!");
			die();
		}
...
if (in_array($format, ['rss', 'atom'], true)) {
	...
	if (!$query->safeForOpml()) {
		Minz_Error::error(FreshRSS_HttpResponseCode::HTTP_404_NOT_FOUND->value, 'OPML not allowed for this user query!');
		die();
	}
