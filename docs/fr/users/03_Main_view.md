...
* par date d’ajout, en utilisant le [format ISO 8601 d’intervalle entre deux dates](https://fr.wikipedia.org/wiki/ISO_8601#Intervalle_entre_deux_dates) : `date:<intervalle-de-dates>`
	...
	* Depuis le plus ancien jusqu'à une période donnée avant maintenant :
		* `!date:P1M` (dynamiquement, plus ancien qu'un mois avant maintenant)
		* `date:P1Y !date:P1M` (dynamiquement, depuis un an avant maintenant jusqu'à un mois avant maintenant)
	* Syntaxe non supportée :
		* `date:/P1M` (dynamiquement depuis un mois avant maintenant ; veuillez utiliser `!date:P1M`)
		* `date:P1Y/P1M` (dynamiquement depuis un an avant maintenant jusqu'à un mois avant maintenant ; veuillez utiliser `date:P1Y !date:P1M`)
